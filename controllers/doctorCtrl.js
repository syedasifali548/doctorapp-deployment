...
const getDoctorInfoController = async (req,res)=>{
    try{
        ...
        res.status(200).send({
            message:"Doctor data fetched successfull!y",
            success:true,
            data:doctor
        })
    }
        ...
        console.log(error)
        res.status(500).send({message:"Errors while fetching doctor account!",error,success:false})
    }
...
const getDoctorByIdController= async (req,res)=>{
  try{
    ...
    console.log(error);
    res.status(500).send({
      message:"getting errors while fetching single doctor",
      success:false
    })
  }
}
